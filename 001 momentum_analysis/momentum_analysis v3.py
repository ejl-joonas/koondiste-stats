def calculate_event_momentum_score(row):
    ...
    is_estonia = team in ["AA", "DA"]
    is_opponent = team in ["DD", "AD"]
...
def analyze_team_stats(df):
    ...
    # Goals
    team_stats['Estonia']['goals'] = df[(df['Põhimoment'].isin(['AA', 'DA'])) & (df['Result'] == 'SHOTGOAL')].shape[0]
    team_stats['Opponent']['goals'] = df[(df['Põhimoment'].isin(['DD', 'AD'])) & (df['Result'] == 'SHOTGOAL')].shape[0]
    
    # Shots on target
    team_stats['Estonia']['shots_on_target'] = df[(df['Põhimoment']!= 'DD') & (df['Result'] == 'SHOTON')].shape[0]
    team_stats['Opponent']['shots_on_target'] = df[(df['Põhimoment']!= 'AA') & (df['Result'] == 'SHOTON')].shape[0]
    
    # Shots off target
    team_stats['Estonia']['shots_off_target'] = df[(df['Põhimoment']!= 'DD') & (df['Result'] == 'SHOTOFF')].shape[0]
    team_stats['Opponent']['shots_off_target'] = df[(df['Põhimoment']!= 'AA') & (df['Result'] == 'SHOTOFF')].shape[0]
    
    # Entries
    team_stats['Estonia']['entries'] = df[(df['Põhimoment']!= 'DD') & (df['Result'] == 'ENTRY')].shape[0]
    team_stats['Opponent']['entries'] = df[(df['Põhimoment']!= 'AA') & (df['Result'] == 'ENTRY')].shape[0]
    
    # Possession (simplified - based on event count)
    estonia_events = df[df['Põhimoment'] == 'AA'].shape[0]
    opponent_events = df[df['Põhimoment'] == 'DD'].shape[0]
